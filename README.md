...
Decoupled Next.js + Flask paper trading dashboard with a TradingView dark theme, technical analysis, VADER news sentiment, and an autonomous OpenAI trading agent.
...
1. Copy `sp500-backend/.env.example` to `sp500-backend/.env` and set `OPENAI_API_KEY`.
...
- Cycles scan the watchlist, ask OpenAI for BUY/SELL/HOLD + reasoning, apply risk caps, execute paper trades, and append reports to `sp500-backend/data/reports.jsonl`.
